let programs = `bqyqwn (68) -> wscqe, cwxspl, syogw, xnxudsh
ddswb (34)
hnkvw (320)
...
let lowest_weight = 9999;
let bottom_program_children;
let counter = 0;
...
let array = programs.split('\n').forEach(e => {
	let split_array = e.split('->');
	let weight = Number(split_array[0].match(/\d+/)[0]);
	let program_name = split_array[0].match(/^[^\s]+/)[0];;
	if(split_array.length === 2){
		let parent_children = split_array[1].split(',').map(e => e.trim());
		parents[program_name] = parent_children
		children.push(parent_children)
	}
	all_nodes[program_name] = weight;
});

children = [].concat(...children);

Object.keys(parents).forEach(name =>{
	if(!children.includes(name)){
		bottom_program_children = parents[name]
	}
	console.log(name + ': ' + calculateWeight(name, 0));
})

function calculateWeight(node_name, node_weight){
	node_weight += all_nodes[node_name];
	if(parents[node_name]){
		parents[node_name].forEach(e =>{
			node_weight = calculateWeight(e, node_weight);
		})
	}
	return node_weight;
}
...
console.log(bottom_program_children)
