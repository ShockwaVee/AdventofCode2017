...
let array = programs.split('\n').forEach(e => {
	...
	if(split_array.length === 2){
		parent_object = {};
		parent_object['name'] = split_array[0].match(/^[^\s]+/)[0];
		parent_object['children'] = split_array[1].split(',').map(e => e.trim());
		children.push(parent_object.children)
...
parents.forEach(parent =>{
	if(!children.includes(parent.name)){
