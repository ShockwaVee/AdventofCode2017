let instruction_set = `ebu inc 626 if iq < 0
...
let instructions = [];
let registers = {};
let highest_value = -9999;
let highest_value_ever = -9999;

instruction_set.split('\n').forEach(e => {
    let instruction_split = e.split('if');
    let instruction_object = {};
    [instruction_object.target, instruction_object.type, instruction_object.amount] = instruction_split[0].split(' ');
    instruction_object.condition = instruction_split[1].trim();
    instructions.push(instruction_object);
});
...
instructions.forEach(e => {
    if (eval(e.condition)) {
        if (e.type === 'inc') eval(e.target + '+=' + e.amount);
        else if (e.type === 'dec') eval(e.target + '-=' + e.amount);
    }
    if(eval(e.target) > highest_value_ever) highest_value_ever = eval(e.target);
});

instructions.forEach(e => {
    if(eval(e.target) > highest_value) highest_value = eval(e.target);
});

console.log('The highest value ever in your registers is/was: ' + highest_value_ever);